...
- PostgreSQL をそのまま使えるため、`DATA_MODEL.md` のスキーマを変更なく適用できる
...
> ファームウェアに**デバイス認証用の秘密情報**（APIキー・登録キー）を一切持たせない方針のため、以前あった `API_KEY` の `build_flags` 注入は廃止した。Wi-Fi認証情報（SSID・パスワード）は引き続き `build_flags` で注入する（デバイス認証とは別目的の秘密情報であり、廃止対象ではない）。  
...
| メソッド | パス | 説明 |
|---|---|---|
| `POST` | `/start` | `POST /api/enroll` を1回実行し、レスポンスが成功（2xx）の場合のみ送信を開始する（すでに実行中の場合は何もしない）。`enroll` が失敗（`4xx` / `5xx` / ネットワークエラー）した場合は送信を開始せず、`/start` のレスポンスにエラーを返す |
| `POST` | `/stop` | 送信を停止する |
...
async function enroll(): Promise<boolean> {
  ...
  return res.ok  // 2xx のみ true。403（revoked）・400（形式不正）等は false
}
...
app.post('/start', async (_req, res) => {
  ...
  const enrolled = await enroll()  // 実機と同じく起動時に毎回enrollする
  if (!enrolled) {
    // enroll が失敗（403: revoked、400: 形式不正 等）した場合は送信を開始しない
    console.error('[emulator] enroll failed. Not starting.')
    res.status(502).json({ status: 'stopped', message: 'enroll failed' })
    return
  }
...
| ESP32エミュレータの認証モード・コード例 | `Authorization: Bearer` 方式を廃止し、起動時に `POST /enroll` を実行してから `X-Device-MAC` ヘッダーで `readings` を送信する実装例に変更。`/start` は `enroll` のレスポンスが成功（2xx）のときのみ送信を開始するよう修正（失敗時に送信を開始し続けるバグを修正） |
| リポジトリ構成 | `platformio.ini`・`seed.sql` のコメントをキーレス登録の内容に更新。`docs/specs/` 配下の実ファイル名（バージョン番号なし）に合わせてツリー図を修正（旧: `DATA_MODEL_v5.md` 等の存在しないファイル名を記載していた誤りを修正） |
