...
│   ├── src/
│   │   └── main.cpp
│   └── platformio.ini      ← APIキー・送信間隔をここで管理
...
│   ├── seed.sql            ← 初期データ・モックデータ（開発用デバイスのAPIキーを含む）
│   └── config.toml
├── shared/
...
- PostgreSQL をそのまま使えるため、`DATA_MODEL_v4.md` のスキーマを変更なく適用できる
...
- **デバイス認証（`api_key_hash` 照合）および `last_seen_at` 更新は Edge Function 内で Service Role Key を使って処理する**（RLS をバイパスするため）
...
APIキーは `platformio.ini` の `build_flags` でファームウェアに注入する。  
...
それ以外（ゾーン割り当て・閾値・植物マスタ等）はアプリ側で完結する。

| ケース | 内容 |
|---|---|
| センサーを物理的に追加・撤去したとき | `readings` に含める `sensor_type` の文字列を追加・削除する |
| API キーを再発行したとき | `platformio.ini` の `API_KEY` を更新して再ビルド |
| Wi-Fi 認証情報を変えたとき | SSID・パスワードを更新して再ビルド |
| Supabase プロジェクトを作り直したとき | `SUPABASE_URL` を更新して再ビルド（ほぼ発生しない） |

### ゾーンとデバイスの割り当て

ファームウェアはゾーンを知らない。ESP32 は API キーのみ送信し、  
バックエンドが `api_key_hash` → `device` → `zone_id` の順で解決する。  
ゾーンの変更はアプリ側（ゾーン管理画面）から `devices.zone_id` を更新するだけでよく、ファームウェアの変更は不要。
...
`seed.sql` に開発用デバイスと API キーをあらかじめ用意する。  
プレーンキー `dev-api-key-001` は開発環境専用であり、本番には流れない。
...
-- 開発用デバイス（プレーンキー: "dev-api-key-001"）
INSERT INTO devices (id, zone_id, name, api_key_hash, firmware_ver) VALUES (
  'devi-0000-0000-0000-000000000001',
  'zone-0000-0000-0000-000000000001',
  '開発用デバイス',
  encode(sha256('dev-api-key-001'::bytea), 'hex'),
  '0.0.1'
);
```
...
エミュレータの環境変数 `DEVICE_API_KEY=dev-api-key-001` と対応させる。
...
| メソッド | パス | 説明 |
|---|---|---|
| `POST` | `/start` | 送信を開始する（すでに実行中の場合は何もしない） |
| `POST` | `/stop` | 送信を停止する |
...
const API_KEY     = process.env.DEVICE_API_KEY ?? 'dev-api-key-001'
...
async function sendReadings() {
  ...
  console.log(`[${new Date().toISOString()}] POST → ${res.status}`)
}
